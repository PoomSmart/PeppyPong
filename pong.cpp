...
bool moveMe(char key)
{
    ...
    {
        // move our bar until it hits the border
        translate(0, j);
    }
    return true;
}
...
bool moveBot()
{
    ...
    if (com_u->y + j > 0 && com_u->y + LENGTH_COM + j < HEIGHT && bx <= WIDTH - GAP_X - 1 && bx > GAP_X)
    {
        if (level < 3)
        {
            // My calculation for which x-position of the ball bot should be aware of
            // Based on trigonometric ball path prediction
            // Appropriate movement direction depends on the ball direction vector
            // It may not be necessary, as we just follow y-position of the ball and then we should be fine
            // However, this locks the pattern of gameplay if both auto and bot mode are activated
            // So, we should add some randomization to it
...
void moveBall(bool *repeat)
{
    ...
    #endif
    // Dirty calculation for ball collision in every case, any better solution?
    if (y < 1 || y > HEIGHT - 2)
    {
        // Ball hits the top or bottom border
        BALL_dy *= -1;
    }
    ...
    {
        // In case the ball moves towards the corner, invert all vectors to prevent crashing
        BALL_dx *= -1;
        BALL_dy *= -1;
    }
    ball_->x += BALL_dx;
...
void welcomeMessage()
{
    ...
    gotoxy(mX - 25, mY - 6); printf(" ���");               
    gotoxy(mX - 25, mY - 5); printf("| � \\��� � �� � �� �  �");
    gotoxy(mX - 25, mY - 4); printf("|  �/ -�) '� \\ '� \\ || |");
    gotoxy(mX - 25, mY - 3); printf("|�| \\���| .��/ .��/\\�, |");
    gotoxy(mX - 25, mY - 2); printf("        |�|  |�|   |��/");
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_INTENSITY);
    gotoxy(mX + 19, mY - 1); printf("By PoomSmart");
    setDefaultColor();
    gotoxy(mX - 28, mY); printf("���������o���������o���������o���������o���������o���������");
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_GREEN | FOREGROUND_INTENSITY);
    gotoxy(mX - 1, mY + 1); printf(" �����  �����  �����  �����");
    gotoxy(mX - 1, mY + 2); printf("|  �  ||     ||   | ||   ��|");
    gotoxy(mX - 1, mY + 3); printf("|   ��||  |  || | | ||  |  |");
    gotoxy(mX - 1, mY + 4); printf("|��|   |�����||�|���||�����|");
    setDefaultColor();
